...
gate.py's unlock.json and theme.py's theme.json share the same
~/.slate/ directory and the same risk (a stray test-set passphrase or
theme preference would otherwise land in real local config), so both
are isolated here too, pre-emptively.
"""
import pytest
